...
export async function CellState(canvas: HTMLCanvasElement) {
    ...
    const state = new Uint32Array(GRID_SIZE * GRID_SIZE);
    const stateBuffers = [
        device.createBuffer({
    ...
    for (let i = 0; i < state.length; i += 3) state[i] = 1;
    device.queue.writeBuffer(stateBuffers[0], 0, state);

    for (let i = 0; i < state.length; i++) state[i] = i % 2;
    device.queue.writeBuffer(stateBuffers[1], 0, state);
    ...
    function loop() {
        ...
        pass.setBindGroup(0, bindGroups[steps % 2]);
        pass.draw(vertices.length / 2, GRID_SIZE * GRID_SIZE);
        pass.end();

        device.queue.submit([encoder.finish()]);
    }
    const intervalId = setInterval(loop, UPDATE_INTERVAL);
